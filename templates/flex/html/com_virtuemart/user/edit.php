...
<?php if($this->userDetails->user_is_vendor){ ?>
	...
	<button class="btn btn-primary btn-sm" type="submit" onclick="javascript:return myValidator(userForm, true);" ><?php echo $this->button_lbl ?></button><button class="btn btn-dark btn-sm" type="reset" onclick="window.location.href='<?php echo $cancelUrl ?>'" ><?php echo vmText::_('COM_VIRTUEMART_CANCEL'); ?></button></div>
    <?php } ?>
...
if($this->userDetails->virtuemart_user_id!=0) {
	...
	if(!empty($this->manage_link)) {
		echo '<div class="btn sppb-btn-default btn-sm">' . $this->manage_link . '</div>';
	}

	if(!empty($this->add_product_link)) {
		echo '<div class="btn sppb-btn-default btn-sm">' . $this->add_product_link . '</div>';
	}

...
class SppagebuilderAddonVideo extends SppagebuilderAddons {
	...
	public function render() {
		...
		$hideinfo = (isset($settings->hideinfo)) ? $settings->hideinfo : false;
		$no_cookie = (isset($settings->no_cookie)) ? $settings->no_cookie : 0;
		...
		if($url) {
			...
			switch($video['host']) {
				case 'youtu.be':
				$id = trim($video['path'],'/');
				$src = '//www.youtube'.$youtube_no_cookie.'.com/embed/' . $id .'?iv_load_policy=3'. $infoParam;
				break;
					...
					switch( videoObj.host ){
						case "youtu.be":
							var videoId = videoObj.path.trim();
								embedSrc = "//www.youtube"+youtube_no_cookie+".com/embed"+ videoId + "?iv_load_policy=3" + infoParam
							break;

						case "www.youtube.com":
						case "youtube.com":
							var queryStr = videoObj.query.split("=");
								embedSrc = "//www.youtube"+youtube_no_cookie+".com/embed/"+ queryStr[1]+ "?iv_load_policy=3" + infoParam
							break;

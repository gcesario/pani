...
class SppagebuilderAddonOptin_form extends SppagebuilderAddons{
				...
				$output .= '<form class="sppb-optin-form ' . $forminline . ' '.$button_inside.'">';
					if (!$hide_name) {
						$output .= '<div class="sppb-form-group name-wrap">';
							$output .= '<input type="text" name="fname" class="sppb-form-control" placeholder="'. JText::_('COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_NAME') .'" required="required" aria-label="name">';
						$output .= '</div>'; //.sppb-form-group
					}

					$output .= '<div class="sppb-form-group email-wrap">';
						$output .= '<input type="email" name="email" class="sppb-form-control" placeholder="'. JText::_('COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_EMAIL') .'" required="required" aria-label="email">';
					$output .= '</div>'; //.sppb-form-group
	...
	public static function getPageInfoById($pageid, $view_type = 'page'){
		$db = JFactory::getDbo();
	...
	public function css() {
		...
		if($custom_input_style) {
			...
			$css .= $addon_id . ' .sppb-optin-form input::placeholder {';
	...
	public static function getTemplate(){
		...
		<#
			...
			if(data.media_type == "img"){
				...
				if(data.image && data.image.indexOf("https://") == -1 && data.image.indexOf("http://") == -1){
					media = \'<img class="sppb-img-responsive" src="\' + pagebuilder_base + data.image + \'" alt="\' + data.alt_text + \'">\';
				} else if(data.image){
					media = \'<img class="sppb-img-responsive" src="\' + data.image + \'" alt="\' + data.alt_text + \'">\';
				}
			} else{
				media_class = " sppb-optin-form-icon";
				if(data.icon_name){
			...
			var button_text = Joomla.JText._("COM_SPPAGEBUILDER_ADDON_OPTIN_FORM_SUBCSCRIBE");
		...
		#>
		<style type="text/css">
		#sppb-addon-{{ data.id }} .sppb-optin-form-icon {
			font-size: {{ data.icon_size }}px;
			color: {{ data.icon_color }};
		}
		<# if(data.custom_input){ #>
			#sppb-addon-{{ data.id }} .sppb-optin-form input {
			...
			#sppb-addon-{{ data.id }} .sppb-optin-form input::placeholder {
				color: {{ data.custom_input_color }};
				opacity: 1;
			}
		<# } #>
		...
		<div class="sppb-addon sppb-addon-optin-forms {{ data.alignment }} grid{{ grid }}">
			<# if(grid == "ws-4-4-4"){ #>
			...
			<div class="sppb-optin-form-box sppb-row {{ raw_wrap }}">
				...
				<div class="sppb-optin-form-info-wrap media-position-{{ data.media_position || "top" }} {{ info_wrap }}">
					<div class="sppb-optin-form-img-wrap {{ media_class }}">{{{ media }}}</div>
				...
				<div class="sppb-optin-form-content {{ form_wrap }}">
					<form class="sppb-optin-form {{ forminline }} {{button_inside}}">
						<# if (!data.hide_name) { #>
							<div class="sppb-form-group name-wrap">
								<input type="text" name="fname" class="sppb-form-control" placeholder="{{ Joomla.JText._(\'COM_SPPAGEBUILDER_ADDON_AJAX_CONTACT_NAME\') }}" required="required">
							</div>
						<# } #>
